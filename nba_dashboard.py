...
if 'bankroll_data' not in st.session_state:
    st.session_state.bankroll_data = load_bankroll_data()
...
with st.sidebar:
    ...
    # Банкролл
    br = st.session_state.bankroll_data
...
if page == "📊 Overview":
    ...
    games = get_todays_games_from_state()
    bets = load_bets_history()
    todays_bets = {b['game_id']: b for b in bets if b.get('placed_at', '').startswith(date.today().isoformat())}
    
    if games:
        for game in games:
            game_id = game.get('game_id', '')
            bet = todays_bets.get(game_id)
    ...
    with tab1:
        ...
        if st.button("💾 Update Bankroll"):
            ...
            st.session_state.bankroll_data['current'] = new_br
